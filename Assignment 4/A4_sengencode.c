 ...
 * Student Name: Nishchint Dhawan.
 * Student Number: V00882939.
...
int move_to_front(char *str, char c)
{
    ...
    shift = q-p;
    /* shift th   e characters from the second position in str */
 ...
 * @param size: the size of the potential 'shorter' run-length code.
...
int run_length_decode(int *code, int size, int *msg)
{
    ...
    return k;  /* returning the new length of the array after decoding..*/
}
...
int run_length_encode(int *msg, int size, int *code)
{
   ...
   for (i=0; i< size; i++){
    ...
    for (i =0; i< size;i++) {
            ...
            if (counter>=2 && array[i]!=1){
                array[ array2[0] ] = 0;
                array [ array2[1] ] = counter;
                counter -=2;

                for(k = (array2[1]+1); k<=i-1; k++ ) {
                        array[k] = -1;
                }
                j=0;
                counter =0;
  ...
  for (i=0; i< size; i++){
   ...
   return j;    /* returning the length of the new encoded message.*/
 ...
 * @param msg: the original mtf encoded message
 * @param size: the size of the potential 'shorter' run_length code
 * @param code: the run_length encoded message.
...
{
    int  len;
    int *msg2= (int *) malloc(sizeof(int)*MAX_MSG_SIZE ); /* allocate space for the message returned after decoding*/

    len =run_length_decode(code, size, msg2);   /*decode the original run length encoded message and store it in msg2.*/
    int i;
    for (i=0;i<len;i++){    /*loop wise comparison to check if msg2 is same as msg.*/
        if(msg[i]!= msg2[i]){
            return 0;
        }
    }
    ...
    free(msg2);     /*free the space allocated after the comparison is done.*/
